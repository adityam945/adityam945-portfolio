...
const ProjectsCard = ({ data }) => {
    return (
        <div className="card_stylings border border-Blue overflow-hidden h-full">
            <img
                src={data?.image}
                alt="portfolio img"
            ...
            <div id="arrow" className="py-2 px-6 card_stylings hover:-translate-y-10 transition-all ease-in-out duration-500">
                ...
                <div className="text-sm flex flex-wrap gap-3 py-2">
                    {data.technologiesUsed.map((index, key) => <span className='py-2 px-3 text-xs text-Snow bg-EveningBlack rounded-full'>{index.tech}</span>)}
                </div>
            </div>
        </div>
    )
